...
def get_color_block(col, row):
    if col in [1, 2]: return "gray!10"
    if col >= 13 and col <= 18: return "gray!20"
    if col >= 3 and col <= 12 and row <= 7: return "gray!30"
    return "gray!15"
...
for Z, sym, row, col in elements:
    color = get_color_block(col, row)
    x = col - 1
    y = -row + 1
    
    superpos = rf"$| \mathrm{{{sym}}} \rangle \oplus | \overline{{\mathrm{{{sym}}}}} \rangle$"
    
    tex_code += rf"""\draw[fill={color}, draw=black, rounded corners=1pt] ({x},{y}) rectangle ({x+1},{y-1});
\node at ({x+0.5},{y-0.5}) {{\scriptsize {superpos}}};
\node at ({x+0.15},{y-0.15}) {{\tiny {Z}}};
"""

tex_code += r"""
\end{tikzpicture}
}
\caption{Variante C: Algebraische Nomenklatur. Die physikalischen Zustände werden als CPT-Superpositionen dargestellt.}
\end{figure}
"""

os.makedirs(OUT_DIR, exist_ok=True)
with open(os.path.join(OUT_DIR, "ptable_algebraic.tex"), "w", encoding="utf-8") as f:
    f.write(tex_code)
