...
for Z, sym, row, col in elements:
    color = get_color_block(col, row, False)
    x = col - 1
    y = -row + 1 + 10.5 
    
    tex_code += rf"""\draw[fill={color}, draw=black, rounded corners=1pt] ({x},{y}) rectangle ({x+1},{y-1});
\node at ({x+0.5},{y-0.4}) {{\large \textbf{{{sym}}}}};
\node at ({x+0.2},{y-0.15}) {{\tiny {Z}}};
"""

# Annihilation Line
tex_code += r"""
\draw[line width=2pt, dashed, red] (-1, 0) -- (19, 0);
\node[red, right] at (19, 0) {\textbf{Annihilations-Achse ($Z=0$)}};
"""

# Antimatter (Bottom, Mirrored)
for Z, sym, row, col in elements:
    color = get_color_block(col, row, True)
    x = col - 1
    y = row - 1 - 10.5
...
\node at ({x+0.2},{y-0.85}) {{\tiny $\overline{{{Z}}}$}};
"""

tex_code += r"""
\end{tikzpicture}
}
\caption{Variante A: Vollständiges spiegelbildliches duales System.}
\end{figure}
"""

os.makedirs(OUT_DIR, exist_ok=True)
with open(os.path.join(OUT_DIR, "ptable_mirrored.tex"), "w", encoding="utf-8") as f:
    f.write(tex_code)
