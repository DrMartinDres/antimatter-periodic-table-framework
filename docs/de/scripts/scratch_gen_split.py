...
def get_color_block(col, row, is_anti):
    if is_anti:
        if col in [1, 2]: return "purple!30"
        if col >= 13 and col <= 18: return "cyan!30"
        if col >= 3 and col <= 12 and row <= 7: return "blue!30"
        return "gray!30"
    else:
        if col in [1, 2]: return "orange!30"
        if col >= 13 and col <= 18: return "yellow!30"
        if col >= 3 and col <= 12 and row <= 7: return "red!20"
        return "green!20"
...
for Z, sym, row, col in elements:
    c_mat = get_color_block(col, row, False)
    c_anti = get_color_block(col, row, True)
    x = col - 1
    y = -row + 1
    ...
    tex_code += rf"""\fill[{c_mat}, rounded corners=1pt] ({x},{y}) -- ({x+1},{y}) -- ({x+1},{y-1}) -- cycle;
\fill[{c_anti}, rounded corners=1pt] ({x},{y}) -- ({x},{y-1}) -- ({x+1},{y-1}) -- cycle;
\draw[black, rounded corners=1pt] ({x},{y}) rectangle ({x+1},{y-1});
\draw[black, dashed] ({x},{y}) -- ({x+1},{y-1});
\node at ({x+0.7},{y-0.3}) {{\textbf{{{sym}}}}};
\node at ({x+0.3},{y-0.7}) {{\textbf{{{antisym}}}}};
\node at ({x+0.8},{y-0.85}) {{\tiny {Z}}};
\node at ({x+0.2},{y-0.15}) {{\tiny $\overline{{{Z}}}$}};
"""

tex_code += r"""
\end{tikzpicture}
}
\caption{Variante B: Dimensionale Überlagerung (Split-Cell). Materie (oben rechts) und Antimaterie (unten links) in identischen Schalen-Koordinaten.}
\end{figure}
"""

os.makedirs(OUT_DIR, exist_ok=True)
with open(os.path.join(OUT_DIR, "ptable_split.tex"), "w", encoding="utf-8") as f:
    f.write(tex_code)
