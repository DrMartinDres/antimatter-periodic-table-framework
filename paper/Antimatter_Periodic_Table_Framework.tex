...
\date{Review candidate v1.0.0-rc2 -- 26 April 2026}
...
\begin{abstract}
Antimatter does not presently require a new chemical periodic law. Under the standard assumptions of the CPT theorem, isolated anti-atoms are expected to share the internal spectral and orbital structure of their matter counterparts. The empirical evidence, however, is uneven. Antihydrogen is the only pure anti-atomic system that has been produced, trapped, spectroscopically characterized, laser cooled, and gravitationally tested. Antihelium evidence is established for antihelium nuclei, not for a neutral antihelium atom.

This work therefore proposes a CPT-isomorphic evidence-status framework rather than an alternative periodic table. The framework separates theoretically expected periodicity from the experimental realization of individual antimatter systems. Its main representation is a split-cell table that combines classical periodic structure with an explicit evidence layer. Residual notation is reserved for measured or explicitly hypothetical deviations in named observables. The intended contribution is a cautious representation and preparedness framework for future anti-atom, anti-ion, anti-molecule, and astrophysical antimatter searches.
\end{abstract}
...
The statement is deliberately conditional. CPT invariance rests on the usual assumptions of local relativistic quantum field theory and is reviewed, for example, by Lehnert \cite{Lehnert2016}. The framework below does not propose a new test theory for CPT violation. It uses CPT-based expectations as the structural reference and keeps experimental evidence, open systems, and hypothetical residuals in separate layers.
...
Neutral antihydrogen atom & antihydrogen & \anti{H} & Pure anti-atom made from \ap{} and \ep{}; experimental anchor of the framework \cite{Andresen2010,Andresen2011,ALPHA2018}.\\
Antihydrogen ion & positive antihydrogen ion & $\anti{H}^{+}$ & Antiproton plus two positrons; relevant to GBAR-style gravity programs and experimentally harder than neutral \anti{H}; antihydrogen beam production by the GBAR method has been reported \cite{GBAR2023}.\\
...
Generic antimatter-element notation & anti- + element name & \anti{E} or \anti{X} & Working notation, not a new IUPAC naming series. For multi-letter symbols, the overbar covers the whole symbol, as in \anti{He}.\\
\bottomrule
\end{longtable}
...
Antihydrogen is the only pure anti-atom that has been systematically studied. ALPHA reported trapped antihydrogen in 2010 and confinement for 1,000 seconds in 2011 \cite{Andresen2010,Andresen2011}. Subsequent ALPHA measurements observed and characterized the $1S$--$2S$ transition, hyperfine spectrum, fine structure, Lamb-shift-related intervals, and laser cooling of trapped antihydrogen \cite{ALPHA2017,Ahmadi2017Hyperfine,ALPHA2018,ALPHA2020Fine,Baker2021Cooling}. These measurements provide direct spectroscopy tests of the CPT-expected isomorphism. The 2023 ALPHA-g measurement found that released antihydrogen atoms fall in a way compatible with gravitational attraction toward Earth; repulsive \enquote{antigravity} for neutral antihydrogen in the terrestrial field is excluded at the present experimental sensitivity \cite{Anderson2023}. In 2025, ALPHA reported an eightfold increase in trapping rate through Be$^{+}$-assisted positron cooling and the accumulation of more than 15,000 antihydrogen atoms in less than seven hours \cite{Akbari2025}.

Antihydrogen therefore has a special status:

\begin{equation}
\mathbf{S}(\anti{H})=(3,3,3,1,0),
\end{equation}
...
The cosmic matter-antimatter asymmetry remains open: the observable universe is overwhelmingly matter dominated. Baryogenesis reviews describe why the observed asymmetry requires physics beyond a naive matter-antimatter balance \cite{RiottoTrodden1999}. Large nearby antimatter domains are also strongly constrained by gamma-ray and cosmic-microwave-background considerations \cite{Cohen1998}. Cosmic antihelium nucleus or antideuteron signatures would therefore be astrophysically important. GAPS describes a low-energy antinuclei search space where standard astrophysical backgrounds are expected to be small and dark-matter scenarios provide additional motivation \cite{Saffold2021}. Such programs belong in an astrophysical evidence layer, not in chemical periodicity.
...
The charge-product argument justifies the nonrelativistic Coulomb analogy. Full equality of matter and antimatter spectra is a stronger statement, tied to the CPT theorem and to matching internal quantum numbers, boundary conditions, and external fields. In that idealized setting,
...
The classical table remains appropriate while the ordering of relevant levels is preserved. For each pair of compared matter levels define

\begin{equation}
\Delta\epsilon_{\mathrm{M}}
=
\epsilon_{n\ell j}^{\mathrm{M}}(Z)-\epsilon_{n'\ell'j'}^{\mathrm{M}}(Z),
\qquad
\Delta\epsilon_{\overline{\mathrm{M}}}
=
\epsilon_{n\ell j}^{\overline{\mathrm{M}}}(Z)-\epsilon_{n'\ell'j'}^{\overline{\mathrm{M}}}(Z).
\end{equation}
...
Here $\sigma_O$ is the combined experimental and reference uncertainty appropriate to the comparison. The statement $\rho_O$ compatible with zero denotes CPT-compatible equality within finite uncertainty, not an exact zero residual. A significant negative residual $\rho_O<0$ or positive residual $\rho_O>0$ belongs in a residual representation. Missing data are not shifted; they remain marked as unknown. This prevents absence of evidence from being drawn as a physical deviation.
...
for example $\anti{H}[1S\text{--}2S:\rho_{1S\text{--}2S}]$ when a measured pull is being discussed. For the present framework table, antihydrogen spectroscopy is described in words as compatible with zero within uncertainty. For the \anti{He} entry, where only antihelium nucleus evidence exists, the marker remains $K$ for antinucleus status. For higher unrealized anti-atoms, it remains $?$.
...
\anti{H} marker $=$ & ALPHA antihydrogen trapping, spectroscopy, gravity, laser-cooling, and trapping-rate results \cite{Andresen2010,Andresen2011,ALPHA2017,Ahmadi2017Hyperfine,ALPHA2018,ALPHA2020Fine,Baker2021Cooling,Anderson2023,Akbari2025} & Antihydrogen is the experimental anti-atom anchor and is compatible with CPT expectations within measured uncertainty.\\
\anti{He} marker $K$ & STAR antihelium nucleus and antimatter-hypernucleus observations \cite{STAR2011,STAR2024} & The \anti{He} marker denotes antihelium nucleus evidence; neutral antihelium atoms are not established by these sources.\\
...
Higher neutral anti-atoms and anti-molecules & CPT-based structure model plus available antihydrogen-chemistry preparedness literature \cite{Zammit2019,Zammit2025} & These entries are theoretically expected or experimentally open; no neutral anti-atom realization beyond \anti{H} is established in the cited literature.\\
\anti{H} beam and positive ion pathway & ASACUSA antihydrogen source work and GBAR antihydrogen beam production through positronium \cite{Kuroda2014,GBAR2023} & These works support future spectroscopy and gravity programs; they do not establish heavier neutral anti-atoms or antimatter chemistry.\\
\bottomrule
\end{longtable}
...
An antimatter periodic table is not currently supported as a new chemical periodic law, and CPT symmetry gives no reason to expect one. It is useful as a CPT-isomorphic evidence-status framework: it makes visible that antihydrogen is experimentally well anchored, antihelium evidence is presently nuclear rather than neutral-atomic, and the wider anti-atom and antimatter-chemistry landscape remains largely open.

The scientifically robust formulation is:

\begin{quote}
We propose not an alternative periodic law for antimatter, but a status-coded CPT-isomorphic evidence-status framework that represents orbital structure, experimental evidence, open chemistry, astrophysical search domains, and possible residuals as distinct layers.
\end{quote}
