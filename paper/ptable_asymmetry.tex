...
\toprule
\textbf{Observable} & \textbf{left: $\rho_O<0$} & \textbf{compatible with zero} & \textbf{right: $\rho_O>0$} & \textbf{unmeasured / status}\\
\midrule
$m/q$ & -- & \cellcolor{green!45}$\anti{H}$ / $\bar p$ compatible & -- & \cellcolor{orange!35}$\anti{He}$: K; $Z\geq3$: ?\\
$1S$--$2S$ & -- & \cellcolor{green!45}$\anti{H}$ compatible within uncertainty & -- & \cellcolor{gray!8}$Z\geq2$: ?\\
$g$ & -- & \cellcolor{green!45}$\anti{H}$ compatible within uncertainty & -- & \cellcolor{gray!8}$Z\geq2$: ?\\
...
\vspace{0.45em}
\scriptsize
The diagonal means CPT compatibility within finite uncertainty, not an exact zero residual. Left/right entries are used only when a significant negative or positive residual is measured. Missing data remain marked as ? or K and are not interpreted as deviations.
